...
sub_nums_delay_congruent = [101:104 109:112 114 202:204 209:212 214 218:223 230 231 233];
sub_nums_delay_rotated = [305:308 310 311 313 405:408 410 411 413 424:429 434:436];
sub_nums_control_congruent = [101:104 110 120 121 201:204 206 207 216 217];
sub_nums_control_rotated = [301 302 320 322 324 331:334 408:415];

num_subs_delay_congruent = length(sub_nums_delay_congruent);
num_subs_delay_rotated = length(sub_nums_delay_rotated);
...
sub_nums = sub_nums_delay_congruent;
num_subs = num_subs_delay_congruent;
data_path = [pwd '/~delay/model_fitting_2_15_2014'];

% sub_nums = sub_nums_delay_rotated;
% num_subs = num_subs_delay_rotated;
% data_path = [pwd '/~delay/model_fitting_2_15_2014'];

% sub_nums = sub_nums_control_congruent;
% num_subs = num_subs_control_congruent;
% data_path = [pwd '/~control/model_fitting_2_15_2014'];
...
for i = 1:num_subs
    ...
    for j = 1:num_blocks % iterate through blocks
        ...
        % Comment this chunk out if you only want to look at PRA for the best fitting models
%         unix_pra_record(j,i) = pra(j,1);
%         uniy_pra_record(j,i) = pra(j,2);
%         glc_pra_record(j,i) = pra(j,3);

        % What we really want to do here is:
        % if glc wins in transfer, than compare it to the best rule to see
        % how close they are in pra
